...
def lastdays(datee,day):
    if datee == None:
        ...
        if diff.days <= day and diff.days >= 0:
...
def dashboard(request):
    ...
    prop1 = [item for item in pro1 if lastdays(item.domainexpiry,days)]
    prop2 = [item for item in pro2 if lastdays(item.hostexpiry,days)]
    prop3 = [item for item in pro3 if lastdays(item.emailexpiry,days)]

    context = {'domain':prop1 , 'host':prop2 , 'email': prop3, 'r' : days}
